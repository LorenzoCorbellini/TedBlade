/**
 * @description Questa lambda restituisce gli speakers presi da Mongodb Atlas con paginazione
...
export const handler = async (event) => {
  try {
    ...
    const collection = dbo.collection("speakers_full_data");
    ...
    const [result, totalDocuments] = await Promise.all([
      collection.find({})
        ...
        .limit(limit)
        .toArray(),
      collection.countDocuments({}) // Numero di speakers nel DB
    ]);
