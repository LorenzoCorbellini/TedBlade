...
##### FROM FILES
tedx_dataset_path = "s3://cloud-mobile-data-1/final_list.csv"
...
## READ THE DETAILS
details_dataset_path = "s3://cloud-mobile-data-1/details.csv"
...
## READ TAGS DATASET
tags_dataset_path = "s3://cloud-mobile-data-1/tags.csv"
...
# READ RELATED VIDEOS
related_videos_dataset_path = "s3://cloud-mobile-data-1/related_videos.csv"
rv_dataset = spark.read \
    .option("header","true") \
    .option("quote", "\"") \
    .option("escape", "\"") \
    .csv(related_videos_dataset_path)

rv_dataset_agg = rv_dataset.groupBy(col("id")).agg(collect_list("related_id").alias("related"))
rv_dataset_agg.printSchema()
tedx_dataset_agg = tedx_dataset_agg.join(rv_dataset_agg, tedx_dataset_agg._id == rv_dataset_agg.id, "left") \
    .drop("id")
...
write_mongo_options = {
    "connectionName": "TEDx exe",
    "database": "unibg_tedx_2026",
    "collection": "tedx_data_related",
    "ssl": "true",
